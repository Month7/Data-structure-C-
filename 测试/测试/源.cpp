...
typedef struct         //哈夫曼树结构体并声明变量名为hufmtree
{
	char ch;
	float weight;
...
void select(hufmtree tree[], int k, int *p1, int *p2)
{
	int j;
	float small1 = 10000;
	float small2 = 10000;
	*p2 = *p1 = 0;
	for (j = 0; j <k ; j++)    //选出两个权值最小的根结点
		if (tree[j].parent == 0)
...
void huffman(hufmtree tree[])//建立哈夫曼树
{
	...
	for (i = n; i<m; i++)      //进行n-1次合并，产生n-1个新结点
	{
		...
		tree[i].weight = tree[p1].weight + tree[p2].weight; //最小权值和次小权值合并后的新结点i的权值为两个孩子结点权值总和
	}
}//huffman
...
int main()
{
	printf("                            ——哈夫曼编码——\n");
	printf("总共有%d个字符\n", n);
	hufmtree tree[m];                  //哈夫曼树的结点
	codetype code[n];                  //哈夫曼树的叶子
	int i, j;//循环变量
	huffman(tree);//建立哈夫曼树
